...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await PanhandleReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.panhandle.address, '100', '1000', { from: owner });

        await this.panhandle.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setPanhandleReferral(this.referral.address, { from: owner });
